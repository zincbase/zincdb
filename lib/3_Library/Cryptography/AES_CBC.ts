    ...
    export namespace Crypto {
        export namespace AES_CBC {
            ...
            export const encryptAsync = async function(plaintext: Uint8Array, keyHex: string, iv: Uint8Array): Promise<Uint8Array> {
                const webCryptoAvailable = await testIfWebCryptoAvailable();
            ...
            export const encryptAsyncUsingWebCrypto = async function(plaintext: Uint8Array, keyHex: string, iv: Uint8Array): Promise<Uint8Array> {
                ...
                const ciphertext = await crypto.subtle.encrypt(<Algorithm>{ name: "AES-CBC", iv: iv }, cryptoKey, plaintext);
            ...
            export const decryptAsyncUsingWebCrypto = async function(ciphertext: Uint8Array, keyHex: string, iv: Uint8Array): Promise<Uint8Array> {
                const cryptoKey = await getWebCryptoKey(keyHex);
                const plainText = await crypto.subtle.decrypt(<Algorithm>{ name: "AES-CBC", iv: iv }, cryptoKey, ciphertext);
                return new Uint8Array(plainText);
            }

            let webCryptoAvailable_Cached: boolean;

            const testIfWebCryptoAvailable = async function(): Promise<boolean> {
                ...
                try {
                    await getWebCryptoKey("00000000000000000000000000000000");
                } catch (e) {
                    webCryptoAvailable_Cached = false;
                    return false;
                }
