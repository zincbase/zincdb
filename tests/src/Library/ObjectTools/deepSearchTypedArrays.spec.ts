namespace ZincDB {
	export namespace ObjectTools {
		describe("deepSearchTypedArrays:", () => {
			it("Returns a list of all underlying ArrayBuffers used deeply in the object tree", () => {
				...
				const results = deepSearchTypedArrays(obj);
			...
			it("Returns an empty array when given undefined", () => {
				expect(deepSearchTypedArrays(undefined)).toEqual([]);
			});
